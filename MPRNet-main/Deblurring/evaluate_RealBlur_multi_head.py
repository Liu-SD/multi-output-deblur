...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        image_names = [f.split('.')[0] for f in os.listdir(gt_path)]

        assert len(image_names) != 0

        single_head_psnr, single_head_ssim, best_psnr, best_ssim = [], [], [], []
        img_files = []
        for image_name in image_names:
            for head in range(10):
                img_files.append((
                    os.path.join(file_path, image_name + f'_h{head}.png'),
                    os.path.join(gt_path, image_name + '.png'),
                ))
        with concurrent.futures.ProcessPoolExecutor(max_workers=60) as executor:
            ...
            with tqdm(total=len(img_files)) as pbar:
                for filename, PSNR_SSIM in zip(img_files, executor.map(proc, img_files)):
                    ...
                    if len(psnrs) == 10:
                        single_head_psnr.append(psnrs)
                        single_head_ssim.append(ssims)
                        best_psnr.append(np.max(psnrs))
                        best_ssim.append(np.max(ssims))
                        psnrs = []
                        ssims = []

        print(dataset)
        print(np.mean(single_head_psnr, axis=0),
              np.mean(single_head_ssim, axis=0),
              np.mean(best_psnr),
              np.mean(best_ssim))
