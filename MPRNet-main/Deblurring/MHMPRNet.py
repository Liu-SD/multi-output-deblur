...
class SAM(nn.Module):
    def __init__(self, n_feat, kernel_size, bias, n_heads=4):
        super(SAM, self).__init__()
        self.conv1 = conv(n_feat, n_feat, kernel_size, bias=bias)
        self.conv2 = conv(n_feat, 3*n_heads, kernel_size, bias=bias)
        self.conv3 = conv(3*n_heads, n_feat, kernel_size, bias=bias)
        self.n_heads = n_heads

    def forward(self, x, x_img):
        x1 = self.conv1(x)
        img = self.conv2(x) + x_img.repeat(1,self.n_heads, 1, 1)
...
class MPRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=96, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=8, kernel_size=3, reduction=4, bias=False, n_heads=4, combinate_heads=False):
        ...
        self.sam12 = SAM(n_feat, kernel_size=1, bias=bias, n_heads=n_heads)
        self.sam23 = SAM(n_feat, kernel_size=1, bias=bias, n_heads=n_heads)

        self.concat12  = conv(n_feat*2, n_feat, kernel_size, bias=bias)
        self.concat23  = conv(n_feat*2, n_feat+scale_orsnetfeats, kernel_size, bias=bias)
        self.tail     = conv(n_feat+scale_orsnetfeats, out_c*n_heads, kernel_size, bias=bias)
    ...
    def forward(self, x3_img):
        ...
        stage1_img = stage1_img.view(-1, self.n_heads, 3, H, W)
        stage2_img = stage2_img.view(-1, self.n_heads, 3, H, W)
        stage3_img = stage3_img.view(-1, self.n_heads, 3, H, W)
        if self.combinate_heads:
            ...
            for i in range(self.n_heads):
                for j in range(i+1):
                    stage1_img_.append((stage1_img[:,i]+stage1_img[:,j])/2)
                    stage2_img_.append((stage2_img[:,i]+stage2_img[:,j])/2)
                    stage3_img_.append((stage3_img[:,i]+stage3_img[:,j])/2)
            stage1_img = torch.stack(stage1_img_, dim=1)
            stage2_img = torch.stack(stage2_img_, dim=1)
            stage3_img = torch.stack(stage3_img_, dim=1)

        return [stage3_img+x3_img.unsqueeze(1), stage2_img, stage1_img]
