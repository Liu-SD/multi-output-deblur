...
def paired_paths_from_lmdb(folders, keys):
    ...
    """
    assert len(folders) == 2 or len(folders) == 3, (
        'The len of folders should be 2 with [input_folder, gt_folder]; '
        'or be 3 with [input_folder, event_folder, gt_folder]. '
        f'But got {len(folders)}')
    assert len(keys) == 2 or len(keys) == 3, (
        'The len of keys should be 2 with [input_key, gt_key]; '
        'or be 3 with [input_key, event_key, gt_key]'
        f'But got {len(keys)}')
    ...
    if len(folders) == 2:
        ...
        input_folder, event_folder, gt_folder = folders
        input_key, event_key, gt_key = keys
    ...
    if not (input_folder.endswith('.lmdb') and gt_folder.endswith('.lmdb')):
        raise ValueError(
            ...
            f'{gt_key}: {gt_folder}')
    if len(folders) == 3 and not event_folder.endswith('.lmdb'):
        raise ValueError(
            f'{event_key} folder should both in lmdb '
            f'formats. But received {event_key}: {event_folder}')
    ...
    if len(folders) == 3:
        with open(osp.join(event_folder, 'meta_info.txt')) as fin:
            event_lmdb_keys = [line.split('.')[0] for line in fin]
    ...
    if len(folders) == 2:
        ...
        if set(input_lmdb_keys) != set(gt_lmdb_keys) or set(input_lmdb_keys) != set(event_lmdb_keys):
            raise ValueError(
                f'Keys in {input_key}_folder, {event_key}_folder and {gt_key}_folder are different.')
        else:
            paths = []
            for lmdb_key in sorted(input_lmdb_keys):
                paths.append(
                    dict([(f'{input_key}_path', lmdb_key),
                        (f'{event_key}_path', lmdb_key),
                        (f'{gt_key}_path', lmdb_key)]))
    return paths
