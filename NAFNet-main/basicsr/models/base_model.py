        ...
        if not strict:
            ...
            for k in common_keys:
                if crt_net[k].size() != load_net[k].size():
                    if False:
                    # if k == 'ending.weight' or k == 'ending.bias':
                        crt_head = crt_net[k].shape[0] // 3
                        load_head = load_net[k].shape[0] // 3
                        logger.warning(
                            f'Duplicate {load_head} head(s) {k} into {crt_head} heads')
                        load_net[k] = torch.cat([load_net[k] for _ in range(crt_head//load_head)], dim=0)
                    else:
                        logger.warning(
