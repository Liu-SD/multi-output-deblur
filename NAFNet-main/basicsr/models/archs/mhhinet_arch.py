...
class SAM(nn.Module):
    def __init__(self, n_feat, kernel_size=3, bias=True, n_heads=4):
...
class MHHINet(nn.Module):

    def __init__(self, in_chn=3, wf=64, depth=5, relu_slope=0.2, hin_position_left=0, hin_position_right=4, n_heads=4, combinate_heads=False):
        super(MHHINet, self).__init__()
        self.depth = depth
        self.n_heads = n_heads
        ...
        for i in reversed(range(depth - 1)):
            ...
            prev_channels = (2**i)*wf
        self.sam12 = SAM(prev_channels, n_heads=n_heads)
        self.cat12 = nn.Conv2d(prev_channels*2, prev_channels, 1, 1, 0)

        self.last = conv3x3(prev_channels, in_chn*self.n_heads, bias=True)
    ...
    def forward(self, x):
        ...
        B, C, H, W = image.shape
        out_1 = out_1.view(B, self.n_heads, C, H, W)
        if self.combinate_heads:
            out_1_ = []
            for i in range(self.n_heads):
                for j in range(i+1):
                    out_1_.append((out_1[:, i] + out_1[:, j]) / 2)
            out_1 = torch.stack(out_1_, dim=1)
        #stage 2
        ...
        out_2 = self.last(x2)
        B, C, H, W = image.shape
        out_2 = out_2.view(B, self.n_heads, C, H, W)
        if self.combinate_heads:
            out_2_ = []
            for i in range(self.n_heads):
                for j in range(i+1):
                    out_2_.append((out_2[:, i] + out_2[:, j]) / 2)
            out_2 = torch.stack(out_2_, dim=1)
        out_2 = out_2 + image.unsqueeze(1)
        return [out_1, out_2]
...
class MHHINetLocal(Local_Base, MHHINet):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        MHHINet.__init__(self, *args, **kwargs)
